...
class ChangementEtat
{
    ...
    public function modifierEtat(Sorties $sorties):void
    {
        ...
        $delaiMax = $today->modify(modifier: '-30 days');

        //7 - ARCHIVE : si la date est supérieure à 1 mois, on archive :
        if ($dateDeLaSortie < $delaiMax) {
            $etatId = $this->entityManager->getReference(entityName: Etat::class, id: 7);
        ...
        {
            $etatId = $this -> entityManager -> getReference(entityName: Etat::class, id: 3);
            $sorties -> setEtat($etatId);
            $this -> entityManager -> flush();
        }
        ...
        {
            $etatId = $this -> entityManager -> getReference(entityName: Etat::class, id: 4);
            $sorties -> setEtat($etatId);
            $this -> entityManager -> flush();
        }
        ...
        if ($today > $dateDeLaSortie)
        {
            $etatId = $this -> entityManager -> getReference(entityName: Etat::class, id: 5);
            $sorties -> setEtat($etatId);
            $this -> entityManager -> flush();
        }
}
